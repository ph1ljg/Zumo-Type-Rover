/* 
* CLowpassFilter2p.cpp
*
* Created: 19/03/2021 12:45:42
* Author: philg
*/
...
template <class T>
CLowPassFilter2p<T>::CLowPassFilter2p() 
{ 
...
// constructor
template <class T>
CLowPassFilter2p<T>::CLowPassFilter2p(float sample_freq, float cutoff_freq) 
{
...
// change parameters
template <class T>
void CLowPassFilter2p<T>::set_cutoff_frequency(float sample_freq, float cutoff_freq) 
{
...
template <class T>
float CLowPassFilter2p<T>::get_cutoff_freq(void) const 
{
    return _params.cutoff_freq;
}

template <class T>
float CLowPassFilter2p<T>::get_sample_freq(void) const 
{
    return _params.sample_freq;
}

template <class T>
T CLowPassFilter2p<T>::apply(const T &sample) 
{
...
template <class T>
void CLowPassFilter2p<T>::reset(void) 
{
    return _filter.reset();
}

template <class T>
void CLowPassFilter2p<T>::reset(const T &value) 
{
    return _filter.reset(value, _params);
}

// Make an instances Otherwise we have to move the constructor implementations to the header file :P
template class CLowPassFilter2p<int>;
template class CLowPassFilter2p<long>;
template class CLowPassFilter2p<float>;
template class CLowPassFilter2p<Vector2f>;
template class CLowPassFilter2p<Vector3f>;
