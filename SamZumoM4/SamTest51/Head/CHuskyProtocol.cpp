...
bool CHuskyProtocol::validateCheckSum()
{
...
bool CHuskyProtocol::husky_lens_protocol_receive(uint8_t data)
{
...
bool CHuskyProtocol::husky_lens_protocol_read_begin(uint8_t command)
{
...
uint8_t CHuskyProtocol::husky_lens_protocol_read_uint8(){
...
int16_t CHuskyProtocol::husky_lens_protocol_read_int16()
{
	if (content_current >= content_end || content_read_end){receive_fail = true; return 0;}
	int16_t result;
	memcpy(&result, receive_buffer + content_current, sizeof(result));
...
int32_t CHuskyProtocol::husky_lens_protocol_read_int32()
{
	if (content_current >= content_end || content_read_end){receive_fail = true; return 0;}
	int32_t result;
	memcpy(&result, receive_buffer + content_current, sizeof(result));
//	if (IS_BIG_ENDIAN())
//		__builtin_bswap32(result);

	content_current += sizeof(result);
	return result;
}

float CHuskyProtocol::husky_lens_protocol_read_float()
{
	if (content_current >= content_end || content_read_end){receive_fail = true; return 0;}
	float result;
	memcpy(&result, receive_buffer + content_current, sizeof(result));
...
bool CHuskyProtocol::husky_lens_protocol_read_end(){
	if (receive_fail)
	{
		receive_fail = false;
		return false;
...
uint8_t* CHuskyProtocol::husky_lens_protocol_write_begin(uint8_t command)
{
	send_fail = false;
...
void CHuskyProtocol::husky_lens_protocol_write_uint8(uint8_t content){
...
void CHuskyProtocol::husky_lens_protocol_write_int16(int16_t content){
	if(send_index + sizeof(content) >= FRAME_BUFFER_SIZE) {send_fail = true; return;}
...
void CHuskyProtocol::husky_lens_protocol_write_int32(int32_t content){
	if(send_index + sizeof(content) >= FRAME_BUFFER_SIZE) {send_fail = true; return;}
...
void CHuskyProtocol::husky_lens_protocol_write_float(float content){
	if(send_index + sizeof(content) >= FRAME_BUFFER_SIZE) {send_fail = true; return;}
//	if (IS_BIG_ENDIAN()){__builtin_bswap32(content);}
	memcpy(send_buffer + send_index, &content, sizeof(content));
	send_index += sizeof(content);
...
void CHuskyProtocol::husky_lens_protocol_write_buffer_uint8(uint8_t *content, uint32_t length)
{
...
int CHuskyProtocol::husky_lens_protocol_write_end(){
