...
private:
	...
	FrontDistance_t m_FrontDistance;
	//functions
public:
	CHeadControl();
	~CHeadControl();
	...
	void CheckForScanTimeout();
	bool GetLidarRange(uint16_t &Distance,uint16_t &Strength,uint16_t &Temperature);
	bool GetLidarRange(uint16_t &Distance);
	void StartFrontScan();
	void ScanFrontPath();
	int8_t GetBestDistance(float& angle_deg, uint16_t &distance,uint8_t StartSector,uint8_t EndSector) const;
	int8_t GetClosestObject(float& angle_deg, uint16_t &distance,uint8_t StartSector,uint8_t EndSector) const;
