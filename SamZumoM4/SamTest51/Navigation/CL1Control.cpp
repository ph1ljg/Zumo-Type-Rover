...
float CL1Control::get_yaw()
{
    float Yaw = Ahrs.GetHeading();
    if (_reverse) {
...
int32_t CL1Control::get_yaw_sensor() const
{
    if (_reverse) {
...
int32_t CL1Control::nav_roll_cd(void) const
{
    float ret;
...
float CL1Control::lateral_acceleration(void) const
{
    return _latAccDem;
}

int32_t CL1Control::nav_bearing_cd(void) const
{
    return wrap_180_cd(RadiansToCentiDegrees(_nav_bearing));
}

int32_t CL1Control::bearing_error_cd(void) const
{
    return RadiansToCentiDegrees(_bearing_error);
}

int32_t CL1Control::target_bearing_cd(void) const
{
...
float CL1Control::turn_distance(float wp_radius) const
{
...
float CL1Control::turn_distance(float wp_radius, float turn_angle) const
{
...
float CL1Control::loiter_radius(const float radius) const
{
...
bool CL1Control::reached_loiter_target(void)
{
    return _WPcircle;
}
...
void CL1Control::_prevent_indecision(float &Nu)
{
...
void CL1Control::update_waypoint(const struct Location &prev_WP, const struct Location &next_WP, float dist_min)
{
...
void CL1Control::update_loiter(const struct Location &center_WP, float radius, int8_t loiter_direction)
{
    struct Location _current_loc;
...
void CL1Control::update_heading_hold(int32_t navigation_heading_cd)
{
...
void CL1Control::update_level_flight(void)
{
    // copy to _target_bearing_cd and _nav_bearing
